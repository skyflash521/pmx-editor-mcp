...
if (-not (Test-Path -LiteralPath $Reference)) { throw "写しが無い: $Reference" }
if (-not (Test-Path -LiteralPath $Candidate)) { throw "比べる画像が無い: $Candidate" }

$shot = [System.Drawing.Image]::FromFile((Resolve-Path -LiteralPath $Reference))
try {
    $bytes = [Convert]::FromBase64String((Get-Content -LiteralPath $Candidate -Raw).Trim())
