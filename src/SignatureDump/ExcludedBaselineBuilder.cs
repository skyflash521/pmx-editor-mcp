    ...
    public static class ExcludedBaselineBuilder
    {
        ...
        private static readonly Dictionary<string, Recording> Recorded =
            new Dictionary<string, Recording>(StringComparer.Ordinal)
            {
                ...
                { "CAP-339", new Recording(CapabilityStatus.Provided, "IPXPmx", "非対応件数: 2。全公開メンバー(型単位)。契約注記: FromStream/ToStreamはファイルパス版で代替し対象外。危険操作(上書き保存)。該当は ToFile(System.String)。危険操作(モデル初期化)。該当は現在のモデルを対象に呼ぶ Clear() で、対象を指定して呼ぶ場合は当たらない。呼び出しには確認が要る") },
