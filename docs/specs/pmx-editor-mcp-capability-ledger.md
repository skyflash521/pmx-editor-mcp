...
- **担当**: 分類が「提供」の能力を受け持つ担当群。モデル / セッション / ビュー / 変形・モーション
...
項目は列挙結果の型名 `name`、`signatures` の各項目は[行キー](#行キー) `key` だけを必須で持つ。
**理由は列挙から導けるので書かない**——書いた理由と算出した理由がずれる余地を残さない。どちらの
配列も、項目を識別子の序数昇順で並べる。読み取りは
...
- 各項目が、下記の理由の**いずれかに当たること**。どれにも当たらない対象は対象外にできない。
