...
公開API列挙は、シグネチャを**行キー**で一意に指す。行キーは**宣言型の名前・メンバー名・総称型
引数の数・引数の型と方向の列**で決まる。総称型引数の数を入れるのは、引数の列が同じでその数だけが
違うオーバーロードを言語が許すためである。**変換演算子——メンバー名が `op_Implicit`・`op_Explicit`
——だけは、これに戻り値の型を加える。** これらがすべて同じで戻り値の型だけが違うオーバーロードを
言語が許すのは変換演算子だけである。加えるかどうかはメンバー名で決まり、衝突の有無では変えない。

## 行が指すシグネチャ

台帳の各行が指すシグネチャは、**対象の列に書かれた名前を公開API列挙と突き合わせて機械的に決める**。
名前の解決は次の順で行い、どれにも当たらない名前は不合格とする。

1. 対象の列を、前後に空白を伴う斜線で分割し、得た各名前を独立に解決する。
2. 名前をまず型の名前として引く。照合は列挙結果の型名との完全一致と、名前空間を落とした名前との
   一致を見る。入れ子の型は列挙結果ではプラス記号で区切られるので、名前空間を落とした名前を作る
   ときに区切りを点へ直して比べる(`PXEventArgs.UIModelMouse` は
   `PXCPlugin.Event.PXEventArgs+UIModelMouse` に当たる)。総称型は、台帳が書く逆引用符と個数の
   接尾辞を落とした名前と、列挙結果から山括弧の型引数を落とした名前とで比べる
   (`IPEVmePrimaryValue` は `PEPlugin.Vme.IPEVmePrimaryValue<TValue>` に当たる)。型引数の数は
   照合に使わないので、数だけが違う同名の型が在れば次の規則で不合格になる。
   **複数の型に当たった名前は指す先が定まらないので不合格とする。**
3. 型として引けなかった名前だけを、最後の点でメンバー名と型名に分け、型名を上と同じ規則で引いて
   から同名の公開メンバーを取る。**型として引けた名前をメンバーとして読み直さない**——入れ子の型を
   メンバーと読み違えるのを防ぐためである。

解決の対象は、名前の引き方で分ける。**型の名前で引いた行は、その型と全基底型が宣言する公開
シグネチャのすべて**とする。**型とメンバー名の組で引いた行は、その型と全基底型が宣言するもののうち、
そのメンバー名を持つ公開シグネチャだけ**とする。基底型は、基底クラスと実装インターフェースの双方を
推移的に辿る。基底型まで見るのは、`IPXPmxViewConnector` のように派生インターフェースの名前で
書かれた行のメンバーが基底の `IPEPMDViewConnector` に宣言されているためで、列挙は宣言型で行を
作るからである。

名前空間をまとめて指す2行は名前へ機械展開できないので、行ごとに述語を置く。PMDレガシーの行が指す
シグネチャは「宣言型の名前が `PEPlugin.Pmd.` で始まるシグネチャ」と「宣言型が `PEPlugin.IPEBuilder`
で、戻り値の型の名前が `PEPlugin.Pmd.` で始まるシグネチャ」の和、SDXの行が指すシグネチャは「宣言型の
名前が `PEPlugin.SDX.` で始まるシグネチャ」とする。この2行が指す**型**は、それぞれの名前空間に属する
公開型のうち列挙型でないものとする。シグネチャの宣言型から逆算しないのは、公開メンバーを1件も
宣言しない型が名前空間に在ると逆算では落ちるからである。列挙型を除くのは、凡例が列挙型を数えないと
定めており、どの行も列挙型を指さないためである。

母集合はこれらの解決結果の和とし、**1つのシグネチャが複数の行に属することを許す**——基底型が宣言
する表示制御を、複数の派生型の行が同じく指すためである。**ただし提供対象のシグネチャが属する提供行の
担当は1つに定まらなければならない。** 行が指す先は上の規則で解決できなければならないが、解決結果が
0件になることは許す——`IPXMorphOffset` と `PXEventArgs` は公開メンバーを自分では宣言しない入れ物の
型である。

## 対象外一覧

台帳が行を作らない対象は、機械可読の一覧 `pmx-editor-mcp-ledger-out-of-scope.json` へ列挙する。
**一覧は型単位とシグネチャ単位の2段で持つ。** シグネチャの差集合だけで照合すると、公開メンバーを
1件も宣言しない型が台帳から漏れても差集合に何も現れず素通りする——`IPXMorphOffset` と
`PXEventArgs` のようにそういう型が実在する——ので、型単位の照合を併せて置く。

ファイルのルートはオブジェクトで、`types` と `signatures` の2つの配列を必須で持つ。`types` の各
項目は列挙結果の型名 `name` と `reason`、`signatures` の各項目は[行キー](#行キー) `key` と
`reason` を必須で持つ。どちらの配列も、
項目を識別子の序数昇順で並べる。読み取りは
**未知のフィールド・欠けたフィールド・型の不一致・空文字列・`name` と `key` の重複を拒否する**。

合格条件は次の3つとする。

- 公開型の集合が、台帳の行が指す型と `types` の和にちょうど一致すること(過不足ゼロ。2つは互いに
  素であること——同じ型を台帳の行と対象外一覧の両方へ載せることを許さない)。
- 公開シグネチャの集合が、母集合と、`types` の型が宣言するシグネチャと、`signatures` の和に
  ちょうど一致すること(過不足ゼロ。3つは互いに素であること)。
- 各項目の `reason` が、下記の順序付きの分類器が算出した最初の理由と一致すること。**記録した理由の
  述語を満たすことだけを見ると、先に評価される理由にも当たる項目を見逃す**ので、算出値との一致を
  条件にする。

理由は次の閉集合とし、挙げた順に評価して最初に該当した一つを採る。

- `enumType`(列挙型): 型が列挙型である。凡例が数えないと定める。型単位のみ。
- `delegateType`(デリゲート型): 型がデリゲートである。凡例が定めるとおり実行時オブジェクトは直接は
  受け渡せない。型単位のみ。
- `route`(経路): メンバーの述語は「メンバー種別がメソッド・プロパティ・フィールドのいずれかで、
  公開引数を持たず、値の型が対象アセンブリの公開型で、かつ宣言型と異なる」。イベントとコンストラクタ
  は種別で除く——イベントはハンドラの登録であって値を返さず、列挙が書き出す値の型はハンドラの
  デリゲート型だからであり、コンストラクタは自分の型を作るからである。宣言型と異なることまで見るのは、
  引数のないコンストラクタや `Clone` が自分の型を返して経路に見えるのを防ぐためである。型の述語は
  「公開メンバーを1件以上宣言し、そのすべてがメンバーの述語を満たす」か「公開メンバーを1件も宣言
  せず、基底型を1つ以上持ち、その**すべて**が型の述語を満たす」。凡例が定めるとおり、能力は参照先の
  型の行で計上する。
- `argumentOnly`(引数専用型): その型を宣言型とするシグネチャが母集合に1件も無く、かつその型自身が
  宣言するシグネチャを除くと、その型が他のシグネチャの引数の型としてだけ現れ、そのような引数が1件
  以上ある。型の一致は参照渡しの印を外した完全一致で見て、配列や総称型の型引数の中は見ない。凡例が
  数えないと定める引数専用型に当たる。型単位のみ。

行を作るか対象外一覧へ入れるかは、凡例の定めだけで決める。
...
| CAP-402 | VMEデータ型 | IPEVmeEventElement | 提供 | 変形・モーション | 非対応件数: 6。契約注記: デリゲートを要するシグネチャは非対応 |
...
| CAP-408 | VMEデータ型 | IPEVmeSingleValueElement | 提供 | 変形・モーション | 非対応件数: 6。契約注記: デリゲートを要するシグネチャは非対応 |
| CAP-409 | VMEデータ型 | IPEVmeGroup | 提供 | 変形・モーション | 非対応件数: 8。契約注記: デリゲートを要するシグネチャは非対応 |
| CAP-410 | VMEデータ型 | IPEVmeGroupBone | 提供 | 変形・モーション | 非対応件数: 8。契約注記: デリゲートを要するシグネチャは非対応 |
| CAP-411 | VMEデータ型 | IPEVmeGroupMorph | 提供 | 変形・モーション | 非対応件数: 8。契約注記: デリゲートを要するシグネチャは非対応 |
...
| CAP-433 | VMEデータ型 | IPEVmeBone | 提供 | 変形・モーション | 非対応件数: 6。契約注記: デリゲートを要するシグネチャは非対応 |
| CAP-434 | VMEデータ型 | IPEVmeCameraState | 提供 | 変形・モーション |  |
| CAP-435 | VMEデータ型 | IPEVmeCameraPosition | 提供 | 変形・モーション | 非対応件数: 6。契約注記: デリゲートを要するシグネチャは非対応 |
...
| CAP-443 | VMEデータ型 | IPEVmeLight | 提供 | 変形・モーション | 非対応件数: 6。契約注記: デリゲートを要するシグネチャは非対応 |
