...
5. **ビュー**: まず**判定材料を作る**——空モデルには何も描かれず視点の違いを判別
   ...
   **画像を返すツールは3系統ある**(PMXビュー・変形ビュー・サブビューのクライアント画像取得。
   ...
   消化しないと完全一致のゲートを満たせない。期待: いずれも画像コンテンツが返り、**視点を変える前に
   取った画像と、変えた後に取った画像が異なる**こと。あわせて、**3頂点の配置が視点に応じた見え方で
   描かれていること**を人が見て確かめる——共通契約仕様書が画像の受入条件として求めるもので、
   描かれている内容が意図どおりかを判定する基準を機械には与えられない(2つの画像が異なることは、
   どちらもビューの内容であることを示さない)。縮小の `warnings`
   (元寸法と縮小後寸法)が付くかは、**記録した実寸の長辺と指定値の大小で決まる**——実寸の長辺が
   指定値を超える呼び出しにだけ付く。既定(512)・指定(256)のそれぞれについて、記録した実寸から
   どちらであるかを判定する。**既定の呼び出しで、MCPクライアントの出力が出力サイズの警告を
   含まないこと**も見る。`maxLongSide` の最大値2048の妥当性は、ビューポートをそれ以上に
...
- 置いてよいのは**実行するコマンド・合格条件・常設の検査へ載せられない手順・不合格時の対応**だけと
  する。検査が何を見るかは、それが照合する仕様書の検査の節が持つ。
- **画面の表示の見た目を合否の材料にする手順を置かない。** エディタの画面操作は
  [操作役のスクリプト](#エディタとホストの操作)がUI Automationで行い、合否は手順が実行する
  コマンド・クライアント・ツールが返す出力と終了コードで決める。**人手の確認を置いてよいのは、
  自動で判定する手段を原理的に作れない確認だけ**とし、そのときは何がなぜ作れないのかを手順へ
  書き添える。手段をまだ用意していないだけのものはこれに当たらず、用意して手順へ落とす。
- **常設の検査はリポジトリへ何も書き出さず、1つのコマンドから走る。** 表の1行を超えて書かない。
- **他の検査から必ず従う検査は置かない。** 落ちる場面が無い。
- **入力から導ける値を追跡下の正本にしない。** 同じ入力から生成器を走らせて一致するなら導ける値で、
  置けば取り直し忘れがずれになる。生成器が追跡下に在るかを問わず、例外を設けない。要るときに
  その場で導く。
