...
namespace PmxEditorMcp.SignatureDump.Tests
{
    public sealed class CommandRunnerTests : IDisposable
    {
        ...
        private static readonly Dictionary<string, string> LedgerRemarks =
            ...
            { "CAP-339", "非対応件数: 2。全公開メンバー(型単位)。契約注記: FromStream/ToStreamはファイルパス版で代替し対象外。危険操作(上書き保存)。該当は ToFile(System.String)。危険操作(モデル初期化)。該当は現在のモデルを対象に呼ぶ Clear() で、対象を指定して呼ぶ場合は当たらない。呼び出しにはアーキテクチャ仕様書の危険操作の公開方針が定める確認が要る" },
        ...
        public void TypeRolesSubcommandRunsTheCollation()
        {
            ...
            File.WriteAllText(rolesPath, "{\"types\":[]}");
            string editorDirectory = CreateEditorDirectory();
